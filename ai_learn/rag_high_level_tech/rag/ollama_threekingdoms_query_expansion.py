"""
三国知识库 + 查询扩展（Query Expansion）
=========================================

在 Rerank 基础上，增加查询扩展功能。
演示查询扩展如何提升召回率和准确率。

查询扩展方法：
1. 手工规则扩展（同义词表）
2. LLM 查询重写

特点：
- 对比查询扩展前后的检索效果
- 专门针对三国知识库优化
- 可视化展示扩展后的查询词

作者: Claude Code Assistant
日期: 2026-01-28
"""

import os
import json
import pickle
from typing import List, Dict, Set
from pathlib import Path
import re
import requests

import numpy as np
import faiss

# ============================================================================
# 配置
# ============================================================================

OLLAMA_BASE_URL = "http://localhost:11434"
EMBEDDING_MODEL = "nomic-embed-text"
CHAT_MODEL = "deepseek-r1:1.5b"

DOCS_DIR = "knowledge_threekingdoms"
INDEX_PREFIX = "threekingdoms"
CHUNK_SIZE = 800
CHUNK_OVERLAP = 100
COARSE_TOP_K = 100
FINAL_TOP_K = 5

# ============================================================================
# 三国人物同义词表（手工规则）
# ============================================================================

THREE_KINGDOMS_SYNONYMS = {
    # 蜀国人物
    "刘备": ["玄德", "刘皇叔", "大耳儿", "刘玄德"],
    "关羽": ["云长", "关公", "关二爷", "美髯公", "关云长"],
    "张飞": ["翼德", "张三爷", "环眼贼", "张翼德"],
    "诸葛亮": ["孔明", "卧龙", "诸葛孔明", "军师"],
    "赵云": ["子龙", "常山赵子龙", "赵子龙"],
    "黄忠": ["汉升", "黄汉升", "老将"],
    "马超": ["孟起", "马孟起", "锦马超"],
    "魏延": ["文长", "魏文长"],
    "姜维": ["伯约", "姜伯约"],

    # 魏国人物
    "曹操": ["孟德", "曹孟德", "曹丞相", "阿瞒", "曹公"],
    "曹丕": ["子桓", "曹子桓", "魏文帝"],
    "曹植": ["子建", "曹子建"],
    "司马懿": ["仲达", "司马仲达"],
    "张辽": ["文远", "张文远"],
    "许褚": ["仲康", "许仲康", "虎痴"],
    "典韦": ["恶来"],

    # 吴国人物
    "孙权": ["仲谋", "孙仲谋", "吴侯", "吴大帝"],
    "孙策": ["伯符", "孙伯符", "小霸王"],
    "周瑜": ["公瑾", "周公瑾", "都督"],
    "鲁肃": ["子敬", "鲁子敬"],
    "吕蒙": ["子明", "吕子明"],
    "陆逊": ["伯言", "陆伯言"],
    "黄盖": ["公覆", "黄公覆"],
    "甘宁": ["兴霸", "甘兴霸"],

    # 群雄
    "吕布": ["奉先", "吕奉先", "温侯"],
    "董卓": ["仲颖", "董仲颖", "太师"],
    "袁绍": ["本初", "袁本初"],
    "袁术": ["公路", "袁公路"],
    "貂蝉": ["貂蝉女"],

    # 重要事件/地点
    "赤壁": ["赤壁之战", "火烧赤壁"],
    "桃园": ["桃园三结义", "结义"],
    "五丈原": ["五丈原"],
    "华容道": ["华容道"],
    "荆州": ["荆州城"],
    "成都": ["益州", "蜀都"],
    "许都": ["许昌"],
}

# ============================================================================
# 查询扩展器
# ============================================================================

class QueryExpander:
    """查询扩展器"""

    def __init__(self, method: str = "manual", use_llm: bool = False):
        """
        Args:
            method: 扩展方法 "manual" | "dict"
            use_llm: 是否使用 LLM 重写
        """
        self.method = method
        self.use_llm = use_llm
        self.base_url = OLLAMA_BASE_URL
        self.model = CHAT_MODEL

    def _manual_expand(self, query: str) -> str:
        """手工规则扩展（基于同义词表）"""
        expanded_terms = [query]
        query_lower = query.lower()

        # 查找同义词
        for name, aliases in THREE_KINGDOMS_SYNONYMS.items():
            # 如果查询中包含原名或别名
            if name in query or any(alias in query for alias in aliases):
                # 添加所有相关词
                expanded_terms.append(name)
                expanded_terms.extend(aliases)

        # 去重，保留顺序
        seen = set()
        unique_terms = []
        for term in expanded_terms:
            if term not in seen:
                seen.add(term)
                unique_terms.append(term)

        return " ".join(unique_terms)

    def _llm_rewrite(self, query: str) -> str:
        """使用 LLM 重写查询"""
        prompt = f"""你是一个三国知识专家。请重写以下查询，使其更完整、更准确。

原查询：{query}

要求：
1. 将简称改为全称（如"孔明"改为"诸葛亮"）
2. 添加相关背景信息
3. 保持查询简洁，不超过20字
4. 只输出重写后的查询，不要其他内容

重写后的查询："""

        try:
            response = requests.post(
                f"{self.base_url}/api/generate",
                json={
                    "model": self.model,
                    "prompt": prompt,
                    "stream": False,
                    "options": {"temperature": 0.3, "num_predict": 100}
                },
                timeout=60
            )

            if response.status_code == 200:
                rewritten = response.json().get("response", "").strip()
                # 清理可能的前缀
                rewritten = re.sub(r'^(重写后的查询|查询|：|:)\s*', '', rewritten)
                return rewritten if rewritten else query
        except Exception as e:
            print(f"⚠️  LLM 重写失败: {e}")

        return query

    def expand(self, query: str, verbose: bool = True) -> str:
        """扩展查询

        Args:
            query: 原始查询
            verbose: 是否显示扩展信息

        Returns:
            扩展后的查询
        """
        if verbose:
            print(f"\n📝 原始查询: {query}")

        # 第一步：手工规则扩展
        if self.method == "manual":
            expanded = self._manual_expand(query)
            if verbose and expanded != query:
                print(f"➕ 手工扩展: {expanded}")

        # 第二步：LLM 重写
        if self.use_llm:
            rewritten = self._llm_rewrite(query)
            if verbose and rewritten != query:
                print(f"🤖 LLM 重写: {rewritten}")
            expanded = rewritten

        if verbose:
            if expanded == query:
                print("ℹ️  查询无需扩展")

        return expanded

# ============================================================================
# 复用原有的类（简化）
# ============================================================================

class DocumentLoader:
    """文档加载器"""
    @staticmethod
    def load_txt(file_path: str) -> str:
        with open(file_path, 'r', encoding='utf-8') as f:
            return f.read()

    @staticmethod
    def load_directory(directory: str) -> List[tuple]:
        documents = []
        path = Path(directory)
        for file_path in path.rglob('*.txt'):
            try:
                content = DocumentLoader.load_txt(str(file_path))
                if content.strip():
                    documents.append((file_path.name, content))
                    print(f"✅ 已加载: {file_path.name} ({len(content):,} 字符)")
            except Exception as e:
                print(f"❌ 加载失败 {file_path.name}: {e}")
        return documents

class TextSplitter:
    """文本分割器"""
    def __init__(self, chunk_size: int = CHUNK_SIZE, chunk_overlap: int = CHUNK_OVERLAP):
        self.chunk_size = chunk_size
        self.chunk_overlap = chunk_overlap

    def split_text(self, text: str, metadata: str = "") -> List[Dict]:
        chunks = []
        text = re.sub(r'\n+', '\n', text).strip()
        paragraphs = text.split('\n\n')

        current_chunk = ""
        chunk_id = 0

        for para in paragraphs:
            para = para.strip()
            if not para:
                continue

            if len(current_chunk) + len(para) + 2 <= self.chunk_size:
                current_chunk += para + "\n\n"
            else:
                if current_chunk.strip():
                    chunks.append({
                        "content": current_chunk.strip(),
                        "metadata": metadata,
                        "chunk_id": chunk_id
                    })
                    chunk_id += 1

                if self.chunk_overlap > 0 and current_chunk:
                    overlap_text = current_chunk[-self.chunk_overlap:]
                    current_chunk = overlap_text + para + "\n\n"
                else:
                    current_chunk = para + "\n\n"

        if current_chunk.strip():
            chunks.append({
                "content": current_chunk.strip(),
                "metadata": metadata,
                "chunk_id": chunk_id
            })

        return chunks

    def split_documents(self, documents: List[tuple]) -> List[Dict]:
        all_chunks = []
        for filename, content in documents:
            chunks = self.split_text(content, metadata=filename)
            all_chunks.extend(chunks)
        return all_chunks

class OllamaEmbedding:
    """Ollama Embedding 生成器"""
    def __init__(self):
        self.base_url = OLLAMA_BASE_URL
        self.model = EMBEDDING_MODEL
        try:
            response = requests.get(f"{self.base_url}/api/tags", timeout=5)
            if response.status_code != 200:
                raise Exception("Ollama 未运行")
            print(f"✅ Ollama 连接成功")
        except Exception as e:
            raise Exception(f"无法连接到 Ollama: {e}")

    def get_embedding(self, text: str) -> List[float]:
        try:
            response = requests.post(
                f"{self.base_url}/api/embeddings",
                json={"model": self.model, "prompt": text},
                timeout=60
            )
            if response.status_code == 200:
                return response.json().get("embedding", [])
            return []
        except:
            return []

    def get_embeddings_batch(self, texts: List[str], batch_size: int = 20) -> List[List[float]]:
        """批量获取 Embedding"""
        embeddings = []
        for i in range(0, len(texts), batch_size):
            batch = texts[i:i + batch_size]
            for text in batch:
                emb = self.get_embedding(text)
                if emb:
                    embeddings.append(emb)
                    print(f"✅ [{len(embeddings)}/{len(texts)}] 生成 embedding")
                else:
                    embeddings.append([0.0] * 768)
        return embeddings

class FaissIndex:
    """Faiss 向量索引"""
    def __init__(self, dimension: int = 768):
        self.dimension = dimension
        self.index = None
        self.chunks = []
        self.embeddings = None

    def build_index(self, chunks: List[Dict], embeddings: List[List[float]]):
        self.chunks = chunks
        self.embeddings = np.array(embeddings, dtype='float32')

        if self.embeddings.shape[1] != self.dimension:
            self.dimension = self.embeddings.shape[1]

        self.index = faiss.IndexFlatL2(self.dimension)
        self.index.add(self.embeddings)

        print(f"✅ Faiss 索引构建完成: {len(chunks)} 个文档块")

    def save(self):
        """保存索引"""
        faiss.write_index(self.index, f"{INDEX_PREFIX}_faiss.bin")
        with open(f"{INDEX_PREFIX}_chunks.pkl", 'wb') as f:
            pickle.dump({
                'chunks': self.chunks,
                'embeddings': self.embeddings
            }, f)
        print(f"✅ 索引已保存: {INDEX_PREFIX}_*.bin")

    def load(self):
        """加载索引"""
        self.index = faiss.read_index(f"{INDEX_PREFIX}_faiss.bin")
        with open(f"{INDEX_PREFIX}_chunks.pkl", 'rb') as f:
            data = pickle.load(f)
            self.chunks = data['chunks']
            self.embeddings = data['embeddings']
        self.dimension = self.index.d
        print(f"✅ 索引已加载: {len(self.chunks)} 个文档块")

    def search(self, query_embedding: List[float], top_k: int = COARSE_TOP_K) -> List[Dict]:
        """搜索文档"""
        query_array = np.array([query_embedding], dtype='float32')
        distances, indices = self.index.search(query_array, top_k)

        results = []
        for i, (idx, dist) in enumerate(zip(indices[0], distances[0])):
            if idx < len(self.chunks):
                chunk = self.chunks[idx].copy()
                chunk['coarse_score'] = float(dist)
                chunk['embedding'] = self.embeddings[idx]
                results.append(chunk)

        return results

class Reranker:
    """Rerank 重排序器"""
    def __init__(self, method: str = "vector"):
        self.method = method

    def rerank(self, query: str, query_embedding: List[float], documents: List[Dict]) -> List[Dict]:
        """重排序"""
        if self.method == "vector":
            print("🔄 使用向量相似度进行 rerank...")
            query_vec = np.array(query_embedding).reshape(1, -1)

            for doc in documents:
                doc_vec = doc['embedding'].reshape(1, -1)
                cosine_sim = np.dot(query_vec, doc_vec.T) / (
                    np.linalg.norm(query_vec) * np.linalg.norm(doc_vec)
                )

                l2_dist = doc['coarse_score']
                l2_sim = 1 / (1 + l2_dist)
                doc['rerank_score'] = 0.7 * cosine_sim[0][0] + 0.3 * l2_sim
                doc['cosine_sim'] = cosine_sim[0][0]

            return sorted(documents, key=lambda x: x['rerank_score'], reverse=True)

        elif self.method == "keyword":
            print("🔄 使用关键词匹配进行 rerank...")
            query_keywords = set(query.lower().split())

            for doc in documents:
                content = doc['content'].lower()
                keyword_matches = sum(content.count(kw) for kw in query_keywords)
                l2_sim = 1 / (1 + doc['coarse_score'])
                doc['rerank_score'] = l2_sim * (1 + keyword_matches * 0.1)
                doc['keyword_matches'] = keyword_matches

            return sorted(documents, key=lambda x: x['rerank_score'], reverse=True)

class OllamaChat:
    """Ollama 问答生成器"""
    def __init__(self):
        self.base_url = OLLAMA_BASE_URL
        self.model = CHAT_MODEL

    def generate_answer(self, question: str, context: str) -> str:
        prompt = f"""你是一个专业的三国知识问答助手。请基于以下参考文档回答用户的问题。

参考文档：
{context}

问题：{question}

要求：
1. 答案必须基于参考文档中的信息
2. 如果文档中没有相关信息，请明确说明"提供的文档中没有包含该问题的答案"
3. 保持答案简洁准确

答案："""

        try:
            response = requests.post(
                f"{self.base_url}/api/generate",
                json={
                    "model": self.model,
                    "prompt": prompt,
                    "stream": False,
                    "options": {"temperature": 0.3, "num_predict": 1000}
                },
                timeout=120
            )

            if response.status_code == 200:
                return response.json().get("response", "⚠️  无法生成答案")
            return f"⚠️  生成答案时出错: {response.status_code}"
        except Exception as e:
            return f"⚠️  生成答案时出错: {e}"

# ============================================================================
# 带 Query Expansion 的 RAG 系统
# ============================================================================

class RAGSystemWithQueryExpansion:
    """带查询扩展的 RAG 系统"""

    def __init__(self, use_expansion: bool = True, expansion_method: str = "manual", use_llm_rewrite: bool = False):
        """
        Args:
            use_expansion: 是否使用查询扩展
            expansion_method: 扩展方法 "manual" | "dict"
            use_llm_rewrite: 是否使用 LLM 重写
        """
        self.embedder = OllamaEmbedding()
        self.chat = OllamaChat()
        self.index = FaissIndex()
        self.reranker = Reranker(method="vector")

        # 查询扩展配置
        self.use_expansion = use_expansion
        self.expander = QueryExpander(method=expansion_method, use_llm=use_llm_rewrite) if use_expansion else None

    def build_knowledge_base(self):
        """构建知识库"""
        print("\n" + "=" * 80)
        print("📚 三国知识库构建")
        print("=" * 80)

        # 加载文档
        print("\n第一步：加载文档")
        loader = DocumentLoader()
        documents = loader.load_directory(DOCS_DIR)

        if not documents:
            print(f"❌ 未找到文档，请在 {DOCS_DIR}/ 目录中放入三国文档")
            return

        print(f"\n✅ 共加载 {len(documents)} 个文档")

        # 分割文本
        print("\n第二步：分割文本")
        splitter = TextSplitter()
        chunks = splitter.split_documents(documents)
        print(f"✅ 共分割成 {len(chunks)} 个文本块")

        # 生成 embedding
        print("\n第三步：生成 Embedding")
        texts = [chunk['content'] for chunk in chunks]
        embeddings = self.embedder.get_embeddings_batch(texts)

        # 构建索引
        print("\n第四步：构建 Faiss 索引")
        self.index.build_index(chunks, embeddings)

        # 保存
        print("\n第五步：保存索引")
        self.index.save()
        print("\n✨ 三国知识库构建完成！")

    def load_knowledge_base(self):
        """加载知识库"""
        self.index.load()
        print("✅ 三国知识库已加载")

    def query(self, question: str) -> Dict:
        """查询"""
        print(f"\n{'=' * 80}")
        print(f"🔍 查询: {question}")
        print(f"{'=' * 80}")

        # ========== 查询扩展 ==========
        if self.use_expansion and self.expander:
            print(f"\n🔧 第一步：查询扩展")
            expanded_query = self.expander.expand(question, verbose=True)
        else:
            expanded_query = question

        # 生成查询向量
        query_embedding = self.embedder.get_embedding(expanded_query)
        if not query_embedding:
            return {"error": "无法生成查询向量"}

        # 粗排
        print(f"\n📊 第二步：粗排（向量检索，召回 top-{COARSE_TOP_K}）")
        coarse_results = self.index.search(query_embedding, top_k=COARSE_TOP_K)
        print(f"✅ 粗排完成")

        # 精排
        print(f"\n🎯 第三步：精排（Rerank）")
        reranked_results = self.reranker.rerank(question, query_embedding, coarse_results)
        print(f"✅ 精排完成")

        # 显示对比
        print(f"\n📊 粗排 vs 精排对比（Top-5）:")
        print("\n" + "─" * 80)
        for i in range(min(5, len(reranked_results))):
            coarse = coarse_results[i] if i < len(coarse_results) else None
            rerank = reranked_results[i]

            if coarse:
                print(f"\n  [{i+1}] 粗排: {coarse['metadata'][:30]:30s} (L2: {coarse['coarse_score']:.4f})")
                print(f"      精排: {rerank['metadata'][:30]:30s} (rerank: {rerank.get('rerank_score', 0):.4f})")

                if coarse['metadata'] != rerank['metadata']:
                    print(f"      ⚠️  排名变化！")

        # 最终结果
        final_results = reranked_results[:FINAL_TOP_K]

        # 组装上下文
        context = "\n\n".join([
            f"【来源: {r['metadata'][:50]}】\n{r['content'][:300]}..."
            for r in final_results
        ])

        # 生成答案
        print(f"\n💭 正在生成答案...")
        answer = self.chat.generate_answer(question, context)

        return {
            "answer": answer,
            "sources": final_results,
            "coarse_results": coarse_results[:FINAL_TOP_K],
            "query": question,
            "expanded_query": expanded_query if self.use_expansion else None
        }

# ============================================================================
# 主程序
# ============================================================================

def main():
    print("=" * 80)
    print("🤖 三国知识库 + 查询扩展系统")
    print("=" * 80)
    print("\n📚 基于《三国演义》1.7MB 文本")
    print("🎯 演示查询扩展的效果")

    # 检查 Ollama
    try:
        response = requests.get(f"{OLLAMA_BASE_URL}/api/tags", timeout=5)
        if response.status_code != 200:
            print("\n❌ Ollama 未运行")
            return
    except:
        print("\n❌ 无法连接 Ollama")
        return

    # 选择是否使用查询扩展
    print("\n是否启用查询扩展？")
    print("1. 启用查询扩展（推荐）")
    print("2. 不启用查询扩展")
    print("\n输入 1 或 2（默认 1）: ", end="")

    try:
        choice = input().strip()
        use_expansion = choice != "2"
        print(f"\n✅ {'已启用' if use_expansion else '未启用'}查询扩展")
    except:
        use_expansion = True
        print("\n✅ 已启用查询扩展（默认）")

    # 如果启用扩展，询问扩展方法
    expansion_method = "manual"
    use_llm_rewrite = False

    if use_expansion:
        print("\n选择扩展方法:")
        print("1. 手工规则扩展（基于同义词表，快速）⭐")
        print("2. 手工规则 + LLM 重写（更准确，但较慢）")
        print("\n输入 1 或 2（默认 1）: ", end="")

        try:
            choice = input().strip()
            use_llm_rewrite = (choice == "2")
            print(f"\n✅ 使用 {'手工规则 + LLM' if use_llm_rewrite else '手工规则'} 扩展")
        except:
            use_llm_rewrite = False
            print("\n✅ 使用手工规则扩展（默认）")

    # 创建系统
    try:
        rag = RAGSystemWithQueryExpansion(
            use_expansion=use_expansion,
            expansion_method=expansion_method,
            use_llm_rewrite=use_llm_rewrite
        )
    except Exception as e:
        print(f"\n❌ 初始化失败: {e}")
        print("\n💡 请确保已下载 embedding 模型: ollama pull nomic-embed-text")
        return

    # 构建或加载知识库
    if Path(f"{INDEX_PREFIX}_faiss.bin").exists():
        print(f"\n检测到已有索引，是否重新构建？(y/n): ", end="")
        try:
            choice = input().strip().lower()
            if choice == 'y':
                rag.build_knowledge_base()
            else:
                rag.load_knowledge_base()
        except:
            rag.load_knowledge_base()
    else:
        rag.build_knowledge_base()

    # 交互式问答
    print("\n" + "=" * 80)
    print("💬 开始问答（输入 'quit' 退出）")
    print("=" * 80)

    # 提示示例问题（重点展示查询扩展的效果）
    print("\n💡 示例问题（观察查询扩展的效果）:")
    print("  - 孔明的武器是什么？  (会扩展为：孔明 诸葛亮)")
    print("  - 云长的坐骑？        (会扩展为：云长 关羽)")
    print("  - 孟德如何起家的？   (会扩展为：孟德 曹操)")
    print("  - 文远是谁？         (会扩展为：文远 张辽)")
    print("  - 公瑾怎么死的？     (会扩展为：公瑾 周瑜)")

    while True:
        print("\n" + "─" * 80)
        try:
            question = input("❓ 你的问题: ").strip()

            if not question:
                continue

            if question.lower() in ['quit', 'exit', 'q']:
                print("👋 再见！")
                break

            result = rag.query(question)

            if "error" in result:
                print(f"\n❌ {result['error']}")
                continue

            print("\n" + "─" * 80)
            print("📖 答案:")
            print("─" * 80)
            print(result['answer'])
            print("─" * 80)

            # 如果使用了扩展，显示对比
            if result.get('expanded_query') and result['expanded_query'] != result['query']:
                print(f"\n💡 查询对比:")
                print(f"   原始: {result['query']}")
                print(f"   扩展: {result['expanded_query']}")

        except KeyboardInterrupt:
            print("\n\n👋 再见！")
            break
        except Exception as e:
            print(f"\n❌ 出错了: {e}")
            import traceback
            traceback.print_exc()

if __name__ == "__main__":
    main()
